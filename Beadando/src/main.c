...
void rotation_of_objects(Rotate* rotate) {
	...
	rotate->mars_rotation+=0.1;
	rotate->sun_rotation+=0.05;
	rotate->asteroid_rotation+=0.9;
...
if (action.call_asteroid == TRUE && move->asteroid.x<8000){
	if (asteroid==0){
		move->asteroid.x=-2000;
		move->asteroid.y=1500;
		move->asteroid.z=200;
	}
	move->asteroid.x+=15.0;
	asteroid=1;			}
	else
		if (action.call_asteroid == TRUE && move->asteroid.x>=8000){
		move->asteroid.x=-20000;
		action.call_asteroid = FALSE;
		asteroid=0;}
	else
		if (action.call_asteroid == FALSE){
		move->asteroid.x=-20000;}
